...
farmrouter = function(x){
	...
	if (x==="t"){
		// talk to Grannon
		output(1, "Old Grannon narrows his eyes. \"You... you haven't come to kill my sheep?\" His spear wavers between you two.<br>");
...
frouter1 = function(x,y){
	...
	if (x==="n"){
		...
		output(2, "\"I don't trust in sorcery... but the Geist isn't like men. It's otherworldly. Maybe the Mage can help you fight back.\"");
		output(6, "<span id=menu>Press (<span id=letter>Any</span>) key to continue.</span><br>")
		thread = 6.3;
	} else if (x==="" && y===4){
		if (x==="r") {
...
farm2 = function(x){
	if (userInfo.granflag === true){
		...
		output(2, "\"You seen the Geist yet out there in the Dark Woods?\"<br>" +
			"\"Don't forget to go visit the Mage. Tell him I sent you. If he doesn't kill you for it, maybe he'll agree to help you.\"<br>");
		output(3, "<span id=menu>Press (<span id=letter>Any</span>) key to return to town.</span><br>");
		thread = 1.9;
	} else {
		// grannons dead
...
granfight = function(x){
	clear();
	output(1, "You stride into a clearing and hear a queer, empty howling from a nearby tree. Turning around, you see a gathering black cloud of smoke pouring out of a gap in the tree's trunk");
}
