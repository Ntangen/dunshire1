...
woodsstart = function(){
	...
	clear();
	Meteor.call("acts",x,"events","woods");
	statusupdate();
	...
	if (userInfo.level.level<3){
		output(2, "<span id=menu>Press <span id=enter>S</span> to check your status, <span id=enter>K</span> to review your supplies, <span id=enter>T</span> to return to town, or <span id=enter>Any</span> other key to hunt for beasts.</span><br>");
		...
		if (userInfo.mission==="morgan"){
			output(2, "<span id=menu>Press <span id=enter>S</span> to check your status, <span id=enter>K</span> to review your supplies, <span id=enter>T</span> to return to town, (<span id=enter>Q</span>) to search for Morgan's Quercus tree, or <span id=enter>Any</span> other key to hunt for beasts.</span><br>");
		} else {
			output(2, "<span id=menu>Press <span id=enter>S</span> to check your status, <span id=enter>K</span> to review your supplies, <span id=enter>T</span> to return to town, (<span id=enter>M</span>) to find the Mage's Cave,  or <span id=enter>Any</span> other key to hunt for beasts.</span><br>");
		}
	}
	thread = 1;
}
...
woodsrouter = function(x){
	if (x === 't') townsquare();
		...
		abwoods();
	} else if (x==="m" && (userInfo.mission==="grannon" || userInfo.level.level>3)) {
		mage();
	} else if (x==="k"){
		woodsgear();
	} else if (x==="q" && userInfo.mission==="morgan"){
		if (qturns<2) {
			qturns++;
			woodsencounter();
		} else if (qturns===2) {
			if (Math.random()>0.6) {
				...
				output(2, "Crossing a rise, you see a peculiar movement in the thicket of trees before you. Branches sway - is it the wind? Wary, you rest your hand on your " + userInfo.items.weapon.name + " as you continue east.<br>");
				qturns++;
				setTimeout(function(){ woodsencounter() }, 1000);
			}
		} else if (qturns===3){
			if (Math.random()>0.5){
				quercus();
			} else {
				output(2, "A distant crash in the hills around you. You sniff the wind but smell only cold air. You adjust your " + userInfo.items.armor.name + " and check that it's tight. You note that you've seen no small game about...<br>");
				qturns++;
				setTimeout(function(){ woodsencounter() }, 1000);
			}
		} else if (qturns===4) {
			if (Math.random()>0.4){
				...
				output(2, "In a small clearing, you pass the fresh carcass of a Wereharron. Its bones have been picked clean.<br>");
				qturns++;
				setTimeout(function(){ woodsencounter() }, 1000);
			}
		} else quercus();
...
woodsencounter = function(x){
	Meteor.call("acts",x,"events","woods fight");
	if (userInfo.mission==="grannon" && Math.random()>0.9){
		granfight();
	} else {
		clear();
		output(1, "You hear a rustling nearby. Something draws near. You make ready as you turn your head and see...<br>");
		...
		if (istherefire===true){
			// user has a grenade
			console.log("NaN checkin 2?");
			output(4, "<span id=menu>Press (<span id=letter>R</span>) to attempt to run away, (<span id=letter>M</span>) to invoke magick, (<span id=letter>F</span>) to use Morgan's Fire Chanter, or (<span id=letter>Any</span>) other key to attack.<br></span>");
			thread = 1.1;
		} else {
			// no grenades
			console.log("NaN checkin?");
			output(4, "<span id=menu>Press (<span id=letter>R</span>) to attempt to run away, (<span id=letter>M</span>) to invoke magick, or (<span id=letter>Any</span>) other key to attack.<br></span>");
			thread = 1.1;
		}
	}
}
...
woodsfight = function(x,y){
	...
	if (y === "m"){
		...
		statusupdate();
		if (result === "dead"){
			Meteor.call("acts",userInfo.username,"pubevents","woodsdeath");
			console.log("dead");
	...
	} else if (x==="f" && istherefire()){
		var temp = usegear("fire");
		turns++;
		mhp = mhp-50+monster.defense;
		statusupdate();
		output(2, temp);
		if (mhp<=0){
			// kill
			output(3, "<span id=victory>The " + monster.name + " writhes in flame and collapses to the ground, dead, with a smokey thump!</span><br><br>" +
				"<span id=menu>Press <span id=enter>Any</span> key to continue.</span>");
				thread = 1.2;
		} else {
			// injure
			output(3, "The" + monster.name + " twists in pain, but roars back at you, incensed. It has been injured, but now glares at you, angrier than ever. It readies its counterattack!<br><br>");
			output(3, "<span id=menu>Press <span id=enter>Any</span> key to continue.</span>");
			thread = 1.24
		}
	} else {
...
events = function(x){
	...
	if (rando>=0.98 && flag!="fester"){
		// wounds fester
		Meteor.call("acts",x,"events","woods festering");
	...
	} else if (rando>=0.7 && rando<=0.8){
		// find magic stuff
		Meteor.call("acts",x,"events","woods rubies");
...
woodsrun = function(){
	clear();
	Meteor.call("acts",x,"events","woods run");
...
reward = function(){
	clear();
	Meteor.call("acts",x,"events","woods victory");
	// add gloating here
	...
	output(3, "<span id=menu>Press <span id=enter>Any</span> key to continue.</span>");
	if (flag==="drugs"){
		// if player is addicted to Morgan's drugs, deduct turns at 2x
		userInfo.turnsToday -= (turns * 2);
	} else {
		userInfo.turnsToday -= turns;
	}
	events();
...
lancemagic = function(x){
	...
	if (userInfo.items.magic[x].name==="Thunderous Clap"){
		if (userInfo.turnsToday<=spellz.clap.turnsreq) {
			output(4, "You do not have enough turns left today to invoke this magick.")
			output(5, "<span id=menu>Press <span id=enter>Any</span> key to continue.</span>");
			thread=1.27;
		} else {
			...
			mhp = mhp - attackdamage;
			output(3, "Summoning up the old words, you lance the Thunderous Clap upon the " + monster.name + ", bringing down a calamitous din, causing " + attackdamage + " damage!<br>");
			if (mhp <= 0) return "k";
