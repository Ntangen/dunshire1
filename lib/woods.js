...
woodsstart = function(){
	...
	if (userInfo.level.level<3){
		...
		output(2, "<span id=menu>Press (<span id=enter>Enter</span>) to hunt for beasts, (<span id=enter>S</span>) to check your status, (<span id=enter>K</span>) to review your supplies, (<span id=enter>M</span>) to find the Mage's Cave, or (<span id=enter>T</span>) to return to town.</span><br>");
	}
	thread = 1;
}
...
lancemagic = function(x){
	...
	if (userInfo.items.magic[x].name==="Thunderous Clap"){
		if (userInfo.turnsToday<=spellz.clap.turnsreq) {
			...
			mhp = mhp - attackdamage;
			output(3, "Summoning up the old words, you lance the Thunderous Clap upon the " + monster.name + ", bringing down a calamitous din upon it, causing " + attackdamage + " damage!<br>");
			if (mhp <= 0) return "k";
