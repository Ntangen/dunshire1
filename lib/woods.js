...
woodsencounter = function(x){
	...
	if (userInfo.mission==="grannon" && Math.random()>0.9){
		...
		if (istherefire){
			// user has a grenade
			output(4, +
				"<span id=menu>Press (<span id=letter>R</span>) to attempt to run away, (<span id=letter>M</span>) to invoke magick, (<span id=letter>F</span>) to use Morgan's Fire Chanter, or (<span id=letter>Any</span>) other key to attack.<br></span>");
			thread = 1.1;
		} else {
			// no grenades
			output(4, +
