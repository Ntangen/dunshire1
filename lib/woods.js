...
woodsgear = function(){
	...
	output(4, "<span id=menu>Press the <span id=enter>Number</span> of any item you wish to use, or <span id=enter>Any</span> other key to change your mind.</span><br>");
	thread = 1.25;
	}
}
	...
	if (typeof(parseInt(x))==="number"){
		...
		if (userInfo.items.other[temp]===undefined){
			...
			if (userInfo.items.other[temp].name === "Healing elixir"){
				...
				output(6, "<span id=menu>Press <span id=enter>Any</span> key to continue.</span>");
				thread = 1.23;
			} else if (userInfo.items.other[temp].name === "Precious rubies"){
				userInfo.items.other.splice(temp,1);
				output(4, "You can't use that here.<br>");
				output(6, "<span id=menu>Press <span id=enter>Any</span> key to continue.</span>");
				thread = 1.23;
			} else if (userInfo.items.other[temp].name === "Innoculated kola nuts"){
				userInfo.turnsToday += 5;
				output(4, "You take a handful of Morgan's special kola nuts and crunch down on them. The bitterness almost makes you gag.<br>");
				output (5, "You are energized! Five turns are added to your daily limit!<br>");
				output(6, "<span id=menu>Press <span id=enter>Any</span> key to continue.</span>");
				thread = 1.23;
			}
		}
