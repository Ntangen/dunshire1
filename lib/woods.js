...
lancemagic = function(x){
	...
	if (userInfo.items.magic[x].name==="Thunderous Clap"){
		...
		if (shieldflag){
			...
			output(4, "<span id=menu>Press (<span id=letter>R</span>) to attempt to run away, (<span id=letter>M</span>) to invoke more magick, or (<span id=letter>Any</span>) other key to attack.<br></span>");
			thread = 1.22;
		}
		...
		if (userInfo.turnsToday<=spellz.heal.turnsreq){
			...
			monster.name +"'s hitpoints: " + mhp + "<br>" +
			"------------------------------------------------<br><br>"+
			"<span id=menu>Press (<span id=letter>R</span>) to attempt to run away, (<span id=letter>M</span>) to invoke more magick, or (<span id=letter>Any</span>) other key to attack.<br></span>");
			thread = 1.22;
		}
