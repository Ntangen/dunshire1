...
abbey = function(x){
	...
	Meteor.call("acts",x,"events","abbey");
	if (userInfo.xp < 102) {
...
abbeyup = function(x,y){
	if (x===2){
		...
		output(3, "In addition, you may add 1 point to any one of your following attributes: <span id=letter>L</span>uck, <span id=letter>S</span>trength, <span id=letter>C</span>harisma or <span id=letter>M</span>ysticism.<br>");
...
abbeyrouter = function(x){
	...
	if (CheckAbbey()){
		...
		if (x==="f"){
			...
			output(3, "<span id=quote>Do you require our aid to revive your health, my child?</span> the Cleric asks. <span id=quote>We stand ready to assist. All we ask is a small donation for... expenses. You understand.</span><br>");
			output(4, "Ask the Cleric to heal you completely for <span id=gold>65 gold?</span><br>");
			output (5,"<span id=menu>Press (<span id=letter>N</span>) to change your mind, or (<span id=letter>Any</span>) other key to accept.</span><br>");
			thread = 5.7;
		} else {
			townsquare();
		}
...
abwoods = function(x){
	...
	if (aturns ===1){
		output(2, "You've found the thieves' trail! Their camp can't be far ahead.<br>");
	} else if (aturns===2){
		adiscover();
		return;
	}
	...
	mhp = monster.hp;
	output(3, "As you pursue the trail, a " + monster.name + " approaches! What do you do?<br>");
...
areward = function(x){
	clear();
	output(1, "<span id=victory>You parry their thrust and make a final, skillful blow with your " + userInfo.items.weapon.name + ", slaying the two thieves!</span> They will never burlarize the Abbey ever again.<br>");
	output(2, "You collect their bag, feeling the Cleric's censer inside. You should return it to the Abbey right away!<br>");
