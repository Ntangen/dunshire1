...
magerouter = function(x){
	if (x==="q"){
		...
		output(2, "As your mind becomes stronger, and you gain Mysticism points, I can teach you new curses to lance upon your enemies - or yourself. Each lesson will cost you in gold, <b>plus one precious gem.</b><br>" +
		...
		output(3, "<ul><li>(<span id=letter>1</span>) <span id=menu>Thunderous Clap</span> - your opponent won't know what hit them! (<span id=letter>"+spellz.clap.gold+" Gold</span>)" +
			"<li>(<span id=letter>2</span>) <span id=menu>Egregious Shield</span> - a powerful, temporary shield against enemy attacks in battle (1 Mysticism required) (<span id=letter>"+spellz.shield.gold+" Gold</span>)" +
			"<li>(<span id=letter>3</span>) <span id=menu>Curative Words</span> - restore yourself to full health (once per day) (2 Mysticism required) (<span id=letter>"+spellz.heal.gold+" Gold</span>)</ul>");
...
mpurch = function(x){
	...
	if (x==="n"){
		...
		output(4, "The Mage sighs. \"I have already taught you this sorcery, " + userInfo.username + "... have you forgotten so soon?\"<br>");
		output(5, "<span id=menu>Press <span id=enter>Any</span> key to continue.</span>");
		thread = 7.7;
	} else {
		var temp;
		...
		if (userInfo.gold >= currentmerch.gold) {
			if (findgems()) {
				...
				output(2, "\"Wield this magick wisely, young one...\" says the Mage.<br>");
...
spellz = {
	clap:{
		...
		desc: "A calamitous din befalls your enemy's ears! (Requires 1 turn)",
		cunningreq: 0,
		turnsreq: 1,
		attack: 15,
		gold: 150
	...
	heal:{
		name:"Curative Words",
		desc: "Heal yourself fully (once per day). (Requires 5 turns)",
		cunningreq: 2,
		turnsreq: 5,
		attack: 0,
		gold: 650
	...
	sword:{
		name:"Glowing Sword",
		desc: "Double your weapon's attack value in the next battle. (Requires 8 turns)",
		cunningreq: 3,
		turnsreq: 8,
		attack: 0,
		gold: 900
