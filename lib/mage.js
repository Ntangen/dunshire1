...
magerouter = function(x){
	if (x==="q"){
		clear();
		output(1, "\"Ah... yes...\" it whispers. \"What sorcery I can teach you is limited only by your capacity to learn! Well, that, and how much gold have brought me.\"");
		...
		output(3, "<ul><li>(<span id=letter>1</span>) <span id=menu>Thunderous Clap</span> - your opponent won't know what hit them! (1 Cunning required) (<span id=letter>"+spellz.clap.gold+" Gold</span>)" +
			"<li>(<span id=letter>2</span>) <span id=menu>Egregious Shield</span> - double your defensive bonus during your next battle (2 Cunning required) (<span id=letter>"+spellz.shield.gold+" Gold</span>)" +
			"<li>(<span id=letter>3</span>) <span id=menu>Curative Words</span> - restore yourself to full health (once per day) (3 Cunning required) (<span id=letter>"+spellz.heal.gold+" Gold</span>)</ul><br>");
	...
	if (x==="n"){
		...
		if (userInfo.gold >= currentmerch.gold) {
			if (findgems()) {
				...
				output(1, "The Mage nods solumnly, its shouded head dipping as you hear foreign-sounding chants too low for you to hear. Soon, you feel a spark of inspiration, and the " + currentmerch.name + " spell is suddenly familiar to you!<br>");
