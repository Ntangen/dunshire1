			...
			if (res) {
				...
				if (fortunecomp("luck")){
					...
					if (temp==="k"){
						...
						output(2, "Jumping to their feet, " + target.username + " attacks you first with their " + target.items.weapon.name + ", inflicting " + attackdamage + " damage.<br>");
						output(3, "<span id=menu>Press (<span id=letter>M</span>) to use magick, (<span id=letter>R</span>) to run away, or (<span id=letter>Any</span>) other key to counterattack.</span><br>");
						thread = 8.5
					}
				} else {
					...
					if (temp==="k"){
						...
						output(1, "Your stealth pays off, and you strike " + target.username + " with your " + userInfo.items.weapon.name + ", inflicting " + attackdamage + "damage!<br>");
	...
	if (y===1){
		...
		if (temp==="k"){
			...
			output(1, "Your hitpoints: " + userInfo.hp + "<br>" +
				monster.name +"'s hitpoints: " + target.hp + "<br>"); 
			output(2, "------------------------------------------------<br><br>");
			output(3, "Holding their ground, " + target.username + " heaves at you with their " + target.items.weapon.name + ", inflicting " + attackdamage + " damage!<br>");
		...
		if (x==="r"){
			...
			var temp = dueling("player");
			if (temp==="k"){
				output(1, "With a final parry, you make a final thrust with your " + userInfo.items.weapon.name + " for a crushing blow against " + target.username + "!<br>");
				...
				output(1, "Your hitpoints: " + userInfo.hp + "<br>" +
					monster.name +"'s hitpoints: " + target.hp + "<br>"); 
				output(2, "------------------------------------------------<br><br>");
				output(3, "You lean into the blow, and strike " + target.username + " with your " + userInfo.items.weapon.name + ", inflicting " + attackdamage + "damage!<br>");
...
campwin = function(){
	// player wins
	Meteor.call("userduel",userInfo.username, target,"win", function(err,res){
		console.log("error: " + err);
		console.log("result: " + res);
	});
	var temp = target.gold * 0.5;
	target.gold = temp;
	userInfo.gold += temp
	var temp2 = target.xp * 0.2;
	clear();
	output(1, "While your adversary " + target.username + " lays at your feet, you catch your breath and before looting their body.<br><br>" + 
		"<span id=gold>You find " + temp + " gold on their body!</span><br>");
	output(2, "<span id=gold>You also gain " + temp2 + " experience!</span><br>");
	output(3, "You hope " + target.username + " doesn't hold it against you... but you fear you might've made an enemy today.<br>");
...
campdead = function(){
	// player dies
	Meteor.call("userduel",userInfo, target,"lose", function(err,res){
		console.log("error: " + err);
		console.log("result: " + res);
	});
