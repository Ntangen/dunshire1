...
stalk = function (x){
	clear();
	Meteor.call("acts",x,"events","stalking");
...
stalkfirstround = function(x){
	...
	if (fortunecomp("luck")){
		...
		if (temp==="k"){
			...
			output(3, "<span id=quote>Serves you right, amateur!</span> " + target.username + " spits in your direction, dusting off their pants.<br>");
			...
			output(3, "<span id=menu>Press (<span id=letter>R</span>) to run away, or (<span id=letter>Any</span>) other key to counterattack.</span><br>");
			thread = 8.5
		} else {
...
campfight = function(x,y){
	if (y===1){
		...
		if (temp==="k"){
			...
			output(1, "Dazed, " + target.username + "'s meek thrust bounces harmlessly off your " + userInfo.items.armor.name + ". You laugh cruelly as you prepare your counterattack.<br>");
			output(2, "<span id=menu>Press (<span id=letter>R</span>) to run away, or (<span id=letter>Any</span>) other key to counterattack.</span><br>");
			thread = 8.5
		} else {
			...
			output(5, "<span id=menu>Press (<span id=letter>R</span>) to run away, or (<span id=letter>Any</span>) other key to counterattack.</span><br>");
			thread = 8.5
		}
	} else {
		// player actions
...
campwin = function(){
	// player wins
	Meteor.call("acts",userInfo.username,"pubevents","stalking win");
