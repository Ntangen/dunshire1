...
royale = function(x){
	...
	if (userInfo.mission==="royale" && userInfo.level.level>4){
		output(3, "<span id=menu>Press (<span id=letter>F</span>) to <span id=quest>seek out the fightmaster</span>, (<span id=letter>B</span>) to bet on the bloodsport, or (<span id=letter>R</span>) to return to the Tavern.</span><br>");
	} else {
		// need to add a menu option that's different after level 4
		output(3, "<span id=menu>Press (<span id=letter>F</span>) to seek out the fightmaster, (<span id=letter>B</span>) to bet on the bloodsport, or (<span id=letter>R</span>) to return to the Tavern.</span><br>");
	}
	thread = 3;	
}
...
rrouter = function(x){
	...
	if (x==="f") {
		...
		if (userInfo.level.level===3 && userInfo.mission!="royale"){
			...
			"<span id=quote>\"You? I've heard about you. But I don't care to turn this house into a mortuary. You look weak.\"</span><br>");
			output(3, "<span id=quote>\"But then again... I've heard word of your deeds for the Abbey and Old Grannon. The contenders here always enjoy cutting down fresh meat. Are you now ready to prove yourself in our house?\"</span><br>");
	...
	else { chooseclear(1); chooseclear(2); chooseclear(3); output(3, "Come again?"); 
...
rchallenge = function(x){
	if (x==="y"){
		output(4, "Tox looks surprised. <span id=quote>\"So... you fancy yourself a fighter, then?\"</span> He roars and slams his fist on the table. <span id=quote>\"I like the sound of that!\"</span><br>");
	  ...
	  if (y === 1) {
	    ...
	    if (x === 1) {
	      ...
	      if (temp === "dead") {
	        ...
	        output(2, "As the room fades, you hear the surrounding crowd cheering in adulation for your adversary.<br>");
	        output(3, "<span id=menu>Press <span id=enter>Any</span> key to continue.</span>");
	        thread = 86;
	      } else if (temp === "zip") {
	        output(1, monster.name + " " + monster.strike1 + ", but your armor protects you! You sustain no damage!<br>");
	    ...
	    if (x === 1) {
			...
			output(2, "A short, stout figure, its visage completely hidden in a dark cloak, suddenly appears at the other end of the pit, where before there was nothing. The champion's handlers give... it... a wide berth. \"The Magus\" stands motionless at the other side of the pit.<br>");
