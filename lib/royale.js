	...
	if (y===1){
		...
		if(x===1){
			...
			if (temp==="k"){
				...
				output(1, "You strike at " + monster.name + " with your " + userInfo.items.weapon.name + " and inflict " + result + " damage!<br>");
			...
			if (temp==="dead"){
				...
				if (shieldflag){
				output(1, monster.name + " " + monster.strike1 + ", inflicting " + result + " damage! Your Egregious Shield absorbs part of the blow.<br>" +
					"You hear cheers and boos from the crowd at the clever use of your magick!<br>" +
					"------------------------------------------------<br>");
				} else {
				output(1, monster.name + " " + monster.strike1 + ", inflicting " + result + " damage!<br>" +
		...
		if (x==1){
			...
			var temp = monsterfight(monster);
			if (temp==="dead"){
				output(1, "<span id=death>" + monster.name + "'s daggers are too sharp, and too fast, and you fall to the ground!</span><br>");
		...
		if (x==1){
			...
			output(3, "The Magus shows no sign of movement, giving you an opportunity to strike first!");
			output(4, "<span id=menu>Press (<span id=letter>Any</span>) key to continue.</span>");
			thread=3.47;
			}
			monster = beasts.lev3bs[0];
...
rwins = function(x){
	if (round===1){
		...
		output(2, "\"All right, all right...\" he admits, \"you are truly a worthy warrior, " + userInfo.username + ". Here is your share of tonight's fight purse....<br>");
		...
		output(4, "\"You are welcome back to fight anytime, champion. Cutting down your fellow town wanderers can be a great way of gaining experience... and you can even steal some of their gold! Not that I, er, know anything of that...\"<br>");
...
rrouter2 = function(x){
	...
	if (x==="s"){
		...
		if (temp>0.25){
			output(3, "Tox rolls his eyes and snorts. \"Can't handle the heat of REAL competition, eh " + userInfo.username + "? Fine - go! Come back when you're ready to fight!\"<br>");
			output(4, "The crowd jeers and tosses rotten vegetables as you scurry out of a side entrance, in disgraced retreat.<br>");
			output(5, "<span id=menu>Press <span id=enter>Any</span> key to continue.</span>");
			thread = 2.31;
		} else {
			output(3, "Tox regards you, and slowly narrows his countenance with a cruel grin. \"I don't like your look, " + userInfo.username + ". We won't let you get away that easily today. Bring on the next challenger!");
...
rusegear = function(x){
	if (typeof(parseInt(x))==="number"){
		usegear(x);
		output(6, "<span id=menu>Press <span id=enter>S</span> to check your status, (<span id=letter>K</span>) to review your supplies, (<span id=letter>N</span>) to ask Tox to exit the Battle Royale early, or (<span id=letter>Any</span>) other key to take on the second challenger.</span>");
		thread=3.5;
	} else {
		output(4, "Come again?"); setTimeout(function(){ rrouter2("k")}, 1000)
	}
}
