...
istherefire = function(){
	for (i=0;i<userInfo.items.other.length;i++){
		if (userInfo.items.other[i].name==="Morgan's Fire Chanter"){
			console.log("istherefire checkin TRUE");
			return true;
			break
		}
	}
	console.log("istherefire checkin FALSE");
	return false;
}
...
quercus = function(){
	...
	setTimeout(function(){ output(2, "A giant tree branch swings your way! You manage to duck and roll away, and it misses your head by inches. But looking up, you see a great, hulking tree trunk lumber your way, supported by a churning knot of roots, big and small. The great tree has no face, but its branches are streaked in blood.<br>") }, 500);
