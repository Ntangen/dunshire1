Meteor.methods({
	...
	playerSetup: function(user){
		if (Meteor.isServer){
			...
			console.log("serverProfile insertion: " + result + " and " + result2);
			return result;
		}
	},
		...
		if (Meteor.isServer){
			...
			if (job==="start"){
				...
				for(i=0;i<stub.drinks.recd.length;i++){
					temp += stub.drinks.recd[i].type.name + " from " + stub.drinks.recd[0].sender + "<br>";
				}
				...
				for(i=0;i<stub.drinks.recd.length;i++){
					if (stub.drinks.recd[i].sender.toLowerCase()===input){
						temp.position=i;
						temp.drink=stub.drinks.recd[i];
						return temp;
					} else {
						return false
					}
