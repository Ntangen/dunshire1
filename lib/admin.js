...
Meteor.methods({
	...
	playerSetup: function(user){
		if (Meteor.isServer){
			...
			//console.log("user result: " + result.prof);
			console.log("shadow result: " + result.shadow);
			console.log("bank result: " + result.bank);
			logging(user);
			return result;
		}
	},
	...
	acts: function(user,event,z){
		if (Meteor.isServer){
			if (event==="startcheck"){
				...
				var temp2 = Activity.update({date:monthday},{$set:{activity:newstuff}});
				console.log("activity updated: " + temp2);
			} else if (event==="goings"){
				// user is opponent; z is ??
				console.log(Meteor.user().username + " went to the " + z);
