...
Meteor.methods({
	...
	activitylog: function(date1,date2){
		if (Meteor.isServer){
			//var temp1 = Activity.find({date:date1}).fetch()[0].activity
			var temp2 = Activity.find({date:date2}).fetch()[0].activity
			console.log("activity1: " + temp1);
			console.log("activity2: " + temp2);
			var result = {last:temp1,today:temp2}
			return result;
		}
	}
});
...
logging = function(user){
	...
	var esthrs = date.getUTCHours() - 5;
	var estmins = date.getMinutes();
	var time = esthrs + ":" + estmins;
