	...
	checkUser: function(user,job,input){
		if (Meteor.isServer){
			...
			if (job==="start"){
				...
				console.log("duelghost array 0 index: " + result.duelghost[0].opp);
				}
				...
				temp = month + "-" + day;
				temp.toString();
				var temp2 = Log.find({date:temp}).fetch()[0];
				console.log("temp2: " + temp2);
				if (temp2===undefined){
					Log.upsert({date:temp})
				}
				// var login = {username: user}
				// var temp3 = Log.update({date:temp},{$push: {today:login}});
		...
		if (Meteor.isServer) {
			if (type==="notify"){
				...
				for (i=0;i<temp.length;i++){
					...
					console.log("temp array items pushed: " + i);
				}
