...
Meteor.methods({
	...
	activitylog: function(yest,today){
		if (Meteor.isServer){
			...
			var temp = Meteor.activity.find({date:today}).fetch()[0];
			if (temp===undefined){
				console.log("checkin activitylog");
				if (Meteor.activity.find({date:yest}).fetch()[0]===undefined){
					Meteor.activity.update({date:yest},{$set:{activity: placeholder}});
				}
				var lastresults = Meteor.activity.find({date:yest}).fetch()[0].activity;
				Meteor.activity.update({date:today},{$set:{activity: placeholder}});
				console.log("returning yesterday's activity, set up today's");
				var result = {today:lastresults}
				return result
			} else {
				var temp2 = Meteor.activity.find({date:today}).fetch()[0].activity;
