...
Meteor.methods({
	...
	grabRecord: function(user){
		...
		if (Shadow.find({username:user}).fetch()[0].profile.duel.new.length===0){
			flag2="duel";
		}
		return result;
	},
	...
	userduel: function(player,target,type){
		if (Meteor.isServer) {
			if (type==="notify"){
				...
				var temp = Shadow.find({username:player}).fetch()[0].profile.duel.new;
				console.log("temp array items: " + temp);
				Shadow.update({username:player},{$push:{"profile.duel.old":temp}});
				console.log("old duels after pushing new ones: " + Shadow.find({username:player}).fetch()[0].profile.duel.old);
				Shadow.update({username:player},{$set:{"profile.duel.new":[]}});
				console.log("new duels after clearing: " + Shadow.find({username:player}).fetch()[0].profile.duel.new);
			} else {
				...
				if (type==="win"){
					...
					Shadow.update({username:name},{$set:{"profile.duelflag":true, "profile.duel.new":duelmodel}});
				} else {
					var temp = player.gold*0.5
					duelmodel.result = "w";
