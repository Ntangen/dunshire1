...
greeting = function (x){
	if (x==="h"){
		...
		output(2, "<span id=menu><ul><li><span id=letter>1</span> - The Village of Dunshire" +
			...
			"<li><span id=letter>6</span> - Game concepts" +
			"<li><span id=letter>7</span> - <span id=quest>Exit to the Village of Dunshire</span>" +
			"</ul></span>");
		thread = 0.4;
	} else departure();
}
...
instruct = function(x){
	...
	if (x==="1"){
		...
		output(2, "You advance in rank by completing missions, which you qualify for by gaining experience (usually in the Dark Woods). As you gain experience, the weaker creatures in the Woods will flee, and the more powerful ones will be attracted to you. Beware.<br>");
		output(3, "There is magic in this place. Some of it is good.<br>");
		output(4, "<span id=menu>Press <span id=enter>Any</span> key to continue.</span>");
		thread = 0.41;
	} else if (x==="7"){
		departure();
