	...
	if (x===""){
		...
		Meteor.call('finduser', x, function(err, res){
			...
			if (res) {
				...
				output(3, "<span id=menu>Press <span id=enter>Any</span> key to confirm new player, or <span id=enter>N</span> to try again.</span><br><br><br><br>");
				output(4, login2());
				thread = 0.22;
			}
		});
	...
	if (x===""){
		...
		Meteor.call('grabRecord',u, function(error, result){
			...
			if (temp===1){
				...
				output (2,"<span id=menu>Press <span id=enter>H</span> to hear instructions, or <span id=enter>Any</span> other key to continue on to town.</span><br><br><br><br>");
				output(3, login2());
				thread = 0.3
...
login2 = function(){
	temp = "<span id=gold>Dunshire Beta 2.0:</span> Dunshire has been going through a series of upgrades in the last several days, which unfortunately required resetting all player accounts created prior to January 4th. Apologies - but the backend database is now much more manageable! Additions also included Level 4 of gameplay, updated attack/hitpoint values for most opponents, and improved magick support. This should be the last account reset ever required.<br><br>As always, your feedback is very much appreciated. This is a work in progress. Shoot a drink to <span id=menu>sysop</span>, or drop a line to <span id=menu>sysop@dunshirecones.com</span>. Thanks for playing and good luck!";
	return temp;
}
...
newaccount = function(x, y){
	if (x==="n"){
		...
		if (x===""){
			...
			userInfo.username = u;
			Accounts.createUser({username: u, password:p, profile: userInfo}, function(err){
...
death = function(x){
	clear();
	Meteor.call("acts",x,"events","death");
	statusupdate("dead");
	...
	var temp = Math.random();
	if (temp<0.33){
		userInfo.gold = Math.round(userInfo.gold * 0.40);
	} else if (temp>0.66){
		userInfo.gold = Math.round(userInfo.gold * 0.80);
	} else {
		userInfo.gold = 0;
	}
	Meteor.call("loggingout",userInfo.username);
