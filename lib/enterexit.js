	...
	if (x===""){
		...
		Meteor.call('finduser', x, function(err, res){
			...
			if (res) {
				...
				output(2, "The Innkeeper nods a bit skeptically. <span id=quote>\"Ah, I see. I know " + u +". What's " +u+"'s password?</span><br>");
				output(3, "<span id=menu>(Enter password and press <span id=enter>Enter</span> to continue)</span><br>");
				thread = 0.21
				return;
			} else { 
				console.log("user not found!");
				output(2,"The Innkeeper grunts. <span id=quote>\"Well met, "+u+". Haven't seen you around before. You mean to introduce yourself?\"</span><br><br>");
		...
		Meteor.call('grabRecord',u, function(error, result){
			...
			var temp = dailyreboot();
				if (temp===1){
					...
					output(3, "The Innkeeper's eyes light up in recognition. <span id=quote>\"Of course, " + u + "! I'm sorry I didn't recognize you. Welcome back!\"</span><br>");
	...
	if (x==="n"){
		...
		if (x===""){
			...
			Accounts.createUser({username: u, password:p, profile: newUserStats}, function(err){
					...
					output(4, "<span id=quote>\"Very well. Henceforth, friend, you'll be known as '" + Meteor.user().username + "'.</span><br>");
					output(5, "Your password is: '<strong>" + p + "</strong>'. Don't forget it.<br>");
