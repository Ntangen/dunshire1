...
smithy = function (x){
	clear();
	Meteor.call("acts",x,"events","smithy");
...
smithrouter = function(x){
	if (x === "w") { output(2, "Rolf scowls. <span id=quote>\"Well, all right then... here's what I got today:\"</span><br>" + "<p>" + weaps());
		output (3, "Your current weapon: " + userInfo.items.weapon.name + "<br>");
		output (4, "<span id=menu>Select your item, or press <span id=letter>B</span> if nothing interests you.</span><br>");
		thread = 0.81;
	}
	else if (x === "a") { output(2, "Rolf scowls. <span id=quote>\"Well, all right then... here's what I got today:\"</span><br>" + "<p>" + arms());
		output (3, "Your current weapon: " + userInfo.items.armor.name + "<br>");
		output (4, "<span id=menu>Select your item, or press <span id=letter>B</span> if nothing interests you.</span><br>");
		thread = 0.82;
	} 
	...
	else { chooseclear(2); chooseclear(3); output(2, "Come again?"); setTimeout(function(){ smithy() }, 1000) }
}
...
smithconfirm = function(x, type){
	if (type ==="weap"){
		...
		} else { chooseclear(2); chooseclear(3); output(3, "Come again?"); setTimeout(function(){ smithy() }, 1000) }
...
purch = function(x, type){
	if (x==="n"){
		...
		if (userInfo.gold >= currentmerch.gold) {
			// successful buy
			Meteor.call("acts",x,"events","smith buy");
