...
smithy = function (x){
	...
	if (x === "a") { output(2, "Rolf the Smithy looks up from his workbench.<br>" +
		...
		"<span id=menu>Press (<span id=letter>W</span>) to browse Rolf's weaponry, (<span id=letter>A</span>) to examine his armor, or (<span id=letter>L</span>) to leave.</span>");
	thread = 0.8;		
	}
}
...
purch = function(x, type){
	if (x==="n"){
		...
		if (userInfo.gold >= currentmerch.gold) {
			...
			output(1, "Rolf grunts, handing over the " + currentmerch.name + " and putting your pouch of gold under the counter. \"Wield it well\", stranger.");
			output(2, "<span id=menu>Press <span id=enter>Any</span> key to continue.</span>");
			// warray = undefined;
			thread = 0.83;
		} else {
			// not enough simoleons
			output(1, "Rolf snorts. \"This here ain't a charity! Come back when you have enough gold!\"");
			output(2, "<span id=menu>Press <span id=enter>Any</span> key to continue.</span>");
			thread = 0.83;
		}
	} else if (type === "arm") {
		...
		if (userInfo.gold >= currentmerch.gold) {
			...
			output(1, "Rolf grunts, handing over the " + currentmerch.name + " and putting your pouch of gold under the counter. \"Wear it well, stranger.\"");
			output(2, "<span id=menu>Press <span id=enter>Any</span> key to continue.</span>");
			// aarray = undefined;
			thread = 0.83;
		} else {
...
weaps = function(){
	var temp = "weapons.lev" + userInfo.level.toString();
	warray = eval(temp);
...
arms = function(){
	var temp = "armor.lev" + userInfo.level.toString();
	aarray = eval(temp);
