...
tavern = function(x){
	clear();
	Meteor.call("acts",x,"events","tavern");
...
drinkrouter = function(x){
	Meteor.call("acts",x,"events","tavern drinking");
	if (x==="4"){
		if (userInfo.gold < 25){ insufficientfunds() }
		...
		if (userInfo.gold < 15){ insufficientfunds() }
		...
		if (userInfo.gold < 10){ insufficientfunds() }
		...
		if (userInfo.gold < 5){ insufficientfunds() }
		else {		
...
deantalk = function(){
	Meteor.call("acts",x,"events","tavern talking");
...
minstrel = function(x){
	clear();
	Meteor.call("acts",x,"events","tavern minstrel");
	if (x==="a"){
		if (minst===true){
			...
			if (rando <=0.1){
				...
				var result = Meteor.user().bank.deposit;
				Meteor.call('bankUpdate',user,"dep",result);
		...
		var temp;
		if (findgems()){
			Meteor.call("acts",x,"events","tavern minstrel tip");
...
senddrink = function(x,y,z){
	if (x==="n"){
		...
		clear();
		Meteor.call("acts",x,"events","tavern sending drink");
...
tavstalk = function(x,y){
	if (x===1){
		if (y===userInfo.username){
			clear();
			output(1, "Dean furrows his brow. <span id=quote>You tryin' to make a fool of me? You're standing right there!</span><br>");
			output(2, "<span id=menu>Press <span id=letter>Any</span> key to continue.</span>");
			thread=0.71;
			return;
		}
		...
		clear();
		Meteor.call("acts",x,"events","tavern stalking");
