...
tavmenulist = function(){
	...
	if (userInfo>=2){
		temp += "<li>(<span id=letter>X</span>) inquire casually about another player's whereabouts";
	}
	if (userInfo.level.level>=3){
...
drinkrouter = function(x){
	if (x==="4"){
		...
		statusupdate();	
		clear()
		output(2, "Dean nods. <span id=quote>\"Glass of Top's White Whiskey, coming up! Enjoy.\"</span><br>" +
...
tavstalk = function(x,y){
	if (x===1){
		Meteor.call("finduser",y,function(error,result){
			if (result){
				clear();
				output(1, "As casually as you can, you ask Dean about " + y + "'s whereabouts.<br>");
				if (fortune("char")){
					output(2, "Dean barely looks up from his washing. <span id=quote>Oh, sure, I saw " + y + " here earlier. I think they're sleeping out in the camps outside of town tonight. They're not hard to find.</span><br>");
					output(2, "<span id=menu>Press <span id=letter>Any</span> key to thank Dean and discreetly slip out of the tavern, or <span id=letter>N</span> to forget it and return to your swill.</span>");
					thread=8.2;
					target=y;
				} else {
					// you're shady
					output(2, "Dean stops what he's doing and looks at you suspiciously. <span id=quote>Why are you looking for " + y + "? Wait... I don't want to know.</span> Dean looks both ways and then leans down to speak quietly.<br><br>" +
						"<span id=quote>If you really want to know, I might try to remember...</span> he says, tapping his finger on the bar.<br>");
					output(3, "<span id=menu>Press <span id=letter>S</span> to slide 25 gold pieces across the bar, or <span id=letter>N</span> to change the subject.</span><br>");
					thread=8.1;
				}
			} else {
				output(4, "<span id=menu>Come again? Try the name again, or enter <span id=letter>N</span> to change your mind.</span><br>");
				thread=8;
			}
		});
	} else {
		clear();
		output(1, "Which fellow wanderer do you wish to stalk?<br>" + userlist());
...
tavstalkrouter = function(x,y){
	if (x==="n"){
		...
		if (userInfo.gold<=25){
			...
			output(4, "You slide 25 gold pieces across the bar, and Dean quickly slides them off into his apron.<br>");
			output(5, "Looking around to be sure he's not overheard, Dean leans in close to you again. <span id=quote>I heard that " + y + " is camping just outside of town tonight. Not far from the Graveyard. But... you never heard that from me.</span><br>");
			output(6, "<span id=menu>Press <span id=letter>Any</span> key to thank Dean and discreetly slip out of the tavern, or <span id=letter>N</span> to forget it and return to your swill.</span>");
			thread=8.3;
			target=y;
		}
	}
