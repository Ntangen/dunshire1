bank = function(x){
	clear();
	Meteor.call("acts",x,"events","bank");
...
bankrouter = function(x){
	...
	if (x==="d"){
		...
		if (result === 0){
			...
			output(1, "Your bank balance is currently " + result + " gold.<br>");
	...
	} else { chooseclear(2); chooseclear(3); output(2, "Come again?"); 
	...
	if(!/\D/.test(x) && x>=1) {
		if (x>userInfo.gold){
			...
			var result = Meteor.call('bankUpdate',user,"dep",y, function(error, result){
				...
				userInfo.gold -= y;
				statusupdate();
				var acct = Meteor.user().bank.deposit;
				output(3, "You have deposited " + y + " gold in your account.<br><br>" +
					"You currently have <span id=gold>" + acct + " gold</span> total in your account.<br>");
				output(4, "<span id=menu>Press (<span id=letter>Any</span>) key to continue.</span>");
				thread = 4.2;
				});
			}
...
bwithdraw = function(x){
	var deposit = Meteor.user().bank.deposit;
	if(!/\D/.test(x) && x>=1) {
		if (x>deposit){
			output(3, "The clerk rolls his eyes. <span id=quote>\"I'm afraid you appear to have insufficient funds available.\"</span><br>" +
				"You currently have " + deposit + " gold in your account.<br>");
			output(4, "<span id=menu>Press (<span id=letter>Any</span>) key to continue.</span>");
			thread=4.2;
		} else {
			var y = Math.floor(x);
			var result = Meteor.call('bankUpdate',userInfo.username,"with",y, function(error, result){
