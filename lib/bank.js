...
bankrouter = function(x){
	...
	if (x==="d"){
		...
		output(1, "<span id=quote>\"We here at the Bank of Doworth keep your gold secure. You are guaranteed for any loss. Should you perish in battle, or out in the Dark Forest, the gold on your person will be lost, or even... looted.\"</span> The teller wrinkles his nose at the thought.<br>");
		output(2, "<span id=quote>\"You may deposit your money here without charge, but the Bank charges 2.5% for each withdrawal.</span>\"<br>");
		...
		if (result === undefined){
			output(1, "<span id=quote>I'm sorry - you don't have an account here yet. If you'd like to open one, please make a deposit.</span>");
	...
	if(!/\D/.test(x) && x>=1) {
		if (x>userInfo.gold){
			...
			var result = Meteor.call('bankUpdate',user,"dep",y, function(error, result){
				...
				output(3, "You have deposited " + y + " gold in your account.<br>" +
